...
if data =~ %r{\tMessage: Putting tarball at (http://127.0.0.1:5000/silo-1/\w+)}
  ...
  original_tiffs = files.grep( /ufdc2.xsd/ ).size
  if original_tiffs == 2
    ok "two files in tarball"
    
    # make sure they are different
    image_data = Digest::MD5.hexdigest `curl -s #{url} | tar Oxf - #{ieid}/ufdc2.xsd`
    folder_image_data = Digest::MD5.hexdigest `curl -s #{url} | tar Oxf - #{ieid}/links_\*/www.uflib.ufl.edu/digital/metadata/ufdc2/ufdc2.xsd`
