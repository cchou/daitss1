...
# Tests if an ingested package can be withdrawn by an unknown person
...
if run_withdraw(:ieid => ieid, :type=> 'account', :contact_id => bad_contact_id) == :fail
  ok "Withdraw failed"
else
  ng "Withdraw was a success"
end

if log.grep /intellectual entity #{ieid} cannot be withdrawn by contact #{bad_contact_id}/
